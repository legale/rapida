...
$rapida->db2->query("SELECT 
	...
	LEFT JOIN s_products_categories pc ON p.id = pc.product_id	LEFT JOIN s_brands b on (b.id = p.brand_id)
	LEFT JOIN t_opt t on p.id = t.product_id
	WHERE 
		1
		$stock_filter
		$brand_filter
...
$p_images = $rapida->image->get('products');
...
fwrite($fopen, $string);
	...
	$options = $rapida->features->get_product_options($p['product_id']);
	if (!empty($options)) {
