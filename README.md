...
- Баг или особенность mysql приводила к тому, что select max(pos) from table давал иногда 0, как в базе, а иногда NULL, что вызывало ошибку в поведении метода image->add()
Проблема решена. Запрос изменен на select ifnull(max(pos), 0) as pos from table
