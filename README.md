...
```
Теперь при обращении к адресам админки simpla?module=..... страница загружается корректно, так как nginx сначала вставляет слеш simpla/?module, что заставляет все файлы по относительным путям тянуть из /simpla, а не из /.
