...
class VproductView extends View
{

    function fetch()
    {
        $category_url = $this->root->uri_arr['path']['url'];
        $product_url = $this->root->uri_arr['path']['product_url'];

        if (empty($product_url) || empty($category_url)) {
            return false;
        }


        //выбираем категорию
        $cat = $this->categories->get_category($category_url);
        ...
        //меняем название у товара, добавляя туда название категории в единственном числе
        if ($cat["name_singular"] !== "") {
            $product["name"] = $cat["name_singular"] . " " . $product["name_short"];
        }
        // передаем товар в шаблон
