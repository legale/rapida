...
{	
	...
	{
		// Выбираем свойство
		$query = $this->db->placehold("SELECT id, name, uri, position, in_filter FROM __features WHERE id=? LIMIT 1", (int)$id);
		$this->db->query($query);
	...
	public function add_feature($feature) {
		...
		$feature['uri'] = strtr(translit($feature['name']) , array(' ' => '_', '-' => '_' ) );
		
		//тут удаляем все кроме букв, цифр и _
		$feature['uri'] = preg_replace("/[^\w\d_]+/u", '', $feature['uri']);

		//чтобы избежать повторов uri, проверим на уникальность в базе
		//крутим пока не получим уникальное имя
		while($this->db->query("SELECT uri FROM __features WHERE uri = ?", $feature['uri']) && $this->db->num_rows() !== 0 ){
			if(preg_match('/(.+)([0-9]+)$/', $feature['uri'], $parts)) {
				$feature['uri'] = $parts[1].''.($parts[2]+1);
			} else {
				$feature['uri'] = $feature['uri'].'2';
			}
		}
	...
	{	
		...
		$fid = (int)$feature_id;
		$pid = (int)$product_id;
		$optionhash = hash('md4', 'optionhash'.$val);
		$this->db->query("SELECT `id` FROM __options_uniq WHERE `md4`= (X'$optionhash') ");
		
		//Если запись уже есть - продолжаем работу, если нет добавляем запись в таблицу
		if( $this->db->affected_rows() > 0 ) {
			$vid = $this->db->result('id');
		} else {
			$this->db->query("INSERT INTO __options_uniq SET `val`= '$val', `md4` = (X'$optionhash') ");
			$vid = $this->db->insert_id();
		}

		$query = $this->db->placehold("INSERT INTO __options SET `product_id` = ? , ?! = ?
	...
	public function get_options_uniq($ids = null, $reverse = false){
		...
		$this->db->query("SELECT id, val FROM __options_uniq WHERE 1 $id_filter");
		if ( $reverse === true ) {
	...
	public function get_options($filter = array())
	{
		...
		$select = implode( ', ', array_map(function($a){return '`'.$a.'`';},$filter['feature_id']) );
		...
		for($i = 0; $row = $this->db2->result_array() ; $i++ ){
			...
			foreach($filter['feature_id'] as $fid){
				...
				if(isset($row[$fid]) && !isset($res[$fid][$row[$fid]])){
					...
					$obj->$fid->{$i} = (object)array('feature_id' => $fid, 'vid' => $row[$fid], 'value' => $options_uniq[$row[$fid]]['val']);
				}
			}
