		...
		if ($skip_create_path !== true && !isset($this->tmp[$full_pathx])) {
			...
			if (!@file_exists($full_path) || !@is_writable($full_path)) {
				...
				if (!@file_exists($full_path) || !@is_writable($full_path)) {
					...
					dtimer::log("PLEASE CREATE OR CHMOD " . $full_path . " - 0777 OR ANY WRITABLE PERMISSION!", 92);
				}
			}
...
allow from 127.0.0.1";
		...
		if($skip == false) {
			if(!file_exists($path)) {
				if(!mkdir($path)) {
					...
					dtimer::log("PLEASE CHMOD ".$this->getPath()." - 0777 OR ANY WRITABLE PERMISSION!",92);
				}
			}
		}
	...
	public function get_cache_integer($keyhash){
		...
		//элемент массива содержит данные в виде hex строки, поэтому placehold type ! ключ = (X'значение'),
		$query = "SELECT value FROM __cache_integer WHERE `keyhash` = (X'$keyhash') ";
		$query = $this->db->placehold($query);
