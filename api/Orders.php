...
{
    //в этой переменном у нас записано к какому типу данных относятся поля в таблице s_orders
    private $purchases_types = array(
        'id' => 'i',
        'order_id' => 'i',
        'product_id' => 'i',
        'variant_id' => 'i',
    ...
    function get_orders($filter = array())
    {
        ...
        if (isset($filter['modified_since']))
            $modified_filter = $this->db->placehold('AND o.modified > ?', $filter['modified_since']);
        ...
        $query = $this->db->placehold("SELECT o.id, o.delivery_id, o.delivery_price, o.separate_delivery,
									...
									WHERE 1
									$id_filter $status_filter $user_filter $keyword_filter $label_filter $modified_filter GROUP BY o.id ORDER BY status, id DESC $sql_limit", "%Y-%m-%d");
        $this->db->query($query);
    ...
    public function update_order($id, $order)
    {
        foreach ($order as $k => &$e) {
            if (!isset($this->orders_types[$k])) {
                dtimer::log(__METHOD__ . "field $k is unknown. abort!", 1);
                return false;
            }
            switch ($this->orders_types[$k]) {
                case 'i':
                    $e = (int)$e;
                    break;
    ...
    */
    ...
    {
        dtimer::log(__METHOD__ . " start " . var_export($order_id, true));
        if (empty($order_id)) {
            return array();
        }
        ...
        $q = $this->db->placehold("SELECT ol.order_id, l.id, l.name, l.color, l.pos
    ...
    public function update_purchase($id, $purchase)
    {
        ...
        foreach ($purchase as $k => &$e) {
            if (!isset($this->purchases_types[$k])) {
                dtimer::log(__METHOD__ . "field $k is unknown. abort!", 1);
                return false;
            }
            switch ($this->purchases_types[$k]) {
                case 'i':
                    $e = (int)$e;
                    break;
    ...
    public function add_purchase($purchase)
    {
        ...
        // Не допустить нехватки на складе
        if ($order['closed'] && !empty($purchase['amount']) && $variant['stock'] !== null && $variant['stock'] < $purchase['amount'])
            return false;
    ...
    public function close($order_id)
    {
        ...
        if (!$order['closed']) {
            ...
            foreach ($variants_amounts as $id => $amount) {
                ...
                if (empty($variant) || $variant['stock'] !== null && $variant['stock'] < $amount ) {
                    return false;
                }
            }
            foreach ($purchases as $purchase) {
