...
{capture name=tabs}
    ...
    {if isset($userperm['categories'])}
        <li><a href="?module=CategoriesAdmin">Категории</a></li>
    {/if}
    ...
    {if isset($products_count)}
        {if isset($category['name']) || isset($brand['name'])}
            <h1>{$category['name']} {$brand['name']}
                ({$products_count} {$products_count|plural:'товар':'товаров':'товара'})</h1>
        ...
        <div id="list">
            {foreach $products as $product}
                <div class="{if !$product['visible']}invisible{/if} {if $product['featured']}featured{/if} row">
                    <input type="hidden" name="cur_poss['{$product['id']}']" value="{$product['pos']}">
                    <input type="hidden" name="poss[]" value="{$product['id']}">
                    ...
                    <div class="image cell">
                        <a href="{url module=ProductAdmin id=$product['id']}">
                            {if !empty($product['image'])}
                                <img src="{$product['image']|resize:products:$product['image_id']:35:35}"/>
                            {/if}
                        </a>
                    </div>
                            ...
                            <ul>
                                {foreach $product['variants'] as $variant}
                                    <li {if !$variant@first}class="variant" style="display:none;"{/if}>
                                        ...
                                        <input class="price {if $variant['old_price']>0}old_price{/if}" type="text"
                                               name="price[{$variant['id']}]" value="{$variant['price']}"
                                               {if $variant['old_price']>0}title="Старая цена &mdash; {$variant['old_price']} {$currency['sign']}"{/if} />{$currency['sign']}
                            ...
                            {if $variants_num>1}
                                <div class="expand_variant">
                                    <a class="dash_link expand_variant"
                                       href="#">{$variants_num} {$variants_num|plural:'вариант':'вариантов':'варианта'}
                                        ↓</a>
...
<div id="right_menu">
    ...
    <!-- Фильтры -->
    <ul>
        <li {if !isset($filter)}class="selected"{/if}><a
                    href="{url brand_id=null category_id=null keyword=null page=null filter=null}">Все товары</a></li>
        <li {if isset($filter) && $filter=='featured'}class="selected"{/if}><a
                    href="{url keyword=null brand_id=null category_id=null page=null filter='featured'}">Рекомендуемые</a>
        </li>
        <li {if isset($filter) && $filter=='discounted'}class="selected"{/if}><a
                    href="{url keyword=null brand_id=null category_id=null page=null filter='discounted'}">Со
                скидкой</a></li>
        <li {if isset($filter) && $filter=='visible'}class="selected"{/if}><a
                    href="{url keyword=null brand_id=null category_id=null page=null filter='visible'}">Активные</a>
        </li>
        <li {if isset($filter) && $filter=='hidden'}class="selected"{/if}><a
                    href="{url keyword=null brand_id=null category_id=null page=null filter='hidden'}">Неактивные</a>
        </li>
        <li {if isset($filter) && $filter=='outofstock'}class="selected"{/if}><a
                    href="{url keyword=null brand_id=null category_id=null page=null filter='outofstock'}">Отсутствующие</a>
        </li>
        <li {if isset($filter) && $filter=='no_images'}class="selected"{/if}><a
                    href="{url keyword=null brand_id=null category_id=null page=null filter='no_images'}">Без
                изображений</a></li>
    </ul>
    <!-- Фильтры -->
        ...
        {if isset($categories)}
            <ul>
                ...
                {foreach $categories as $c}
                    <li category_id="{$c['id']}"
                        {if isset($category['id']) && $category['id'] == $c['id']}class="selected"
                        {else}class="droppable category"{/if}><a
                                href='{url keyword=null brand_id=null page=null category_id={$c['id']}}'>{$c['name']}</a>
                    </li>
    ...
    {if isset($brands)}
        ...
        <ul>
            ...
            {foreach $brands as $b}
                <li brand_id="{$b['id']}" {if isset($brand['id']) && $brand['id'] == $b['id']}class="selected"
                    {else}class="droppable brand"{/if}><a
