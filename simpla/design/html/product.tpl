{capture name=tabs}
    <li class="active"><a href="?module=ProductsAdmin">Товары</a></li>
...
<form method="post" class="product" enctype="multipart/form-data">
            ...
            <ul>
                {function name=category_select level=0}
                    {foreach $categories as $cat}
                        <option value="{$cat['id']}" {if $cat['id'] == $selected_id}selected{/if}
                                category_name="{$cat['name']|escape}">{section name=sp loop=$level}&nbsp;&nbsp;&nbsp;&nbsp;{/section}{$cat['name']|escape}</option>
                        {category_select categories=$cat['subcategories'] selected_id=$selected_id  level=$level+1}
                    {/foreach}
                {/function}
                {if isset($product['cats']) && !empty($cats)}
                ...
                {else}
                    <li container="true">
                        <select name="save[cats][]">
                            <option value="{$cat['id']}" category_name="">Не выбрана</option>
                            {category_select categories=$cats selected_id=$pc['category_id']}
                        </select>
                        <a delete_link="true" class="icons delete"></a>
                    </li>
    ...
    <div class="column_right">
        ...
        <div class="block layer">
            ...
            <input type="text" id="related_products_autocomplete" class="input_autocomplete"
    ...
    <div class="block layer">
        ...
        <div class="clear"></div>
        <input class="button_green button_save" type="submit" name="" value="Сохранить"/>
        <span><i add_link="true" link_id="option" class="dash_link"
                 id="add_new_feature">Добавить новое свойство</i></span>
    </div>
